...
function checkNumber(num)
{
	...
	if (reg.test(num) == false) { alert("숫자만 입력할 수 있습니다."); return false; }
...
{
	var reg = new RegExp(/^[0-9]+$/);
	if (reg.test(num) == false) { alert("숫자만 입력할 수 있습니다."); return ''; }		    
	return num;
}
...
function parseHtmlToText(html) {
	...
	console.log('html : ' + html + ' / text : ' + $(ohtml).text());
 ...
 * 셋팅값 변환
 */
function toPercent(v, f) {
	   return (v * 100).toFixed(f);
	}
